...
regex search_vector(string begin) {
...
void SceneParser::build_materials(vector<string> materials_str) {
    ...
    for (long unsigned int i = 0; i<materials_str.size(); i++) {
        ...
        Material m;
        float3 color;
        regex color_rgx = search_vector("color");
        ...
        if (is_light) {
          ...
          regex alpha_rgx = search_float("(roughness)");
          if(regex_search(s.begin(), s.end(), match, alpha_rgx))
            m = mk_material_diffuse(color, stof(match[2]));
          else
            throw_error_mat("no valid roughness|emit in material " + name + ".");
        }
...
void SceneParser::build_meshes(vector<string> meshes_str) {
    ...
    for (long unsigned int i = 0; i <meshes_str.size(); i++) {
        ...
        } else throw_error_mesh("no valid obj_file provided");
    }
}
